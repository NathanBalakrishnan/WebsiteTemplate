...
const CustomizationPanel = ({
  ...
  const handleAddNavItem = () => {
    const newItem = { name: "New Menu", label: "New Menu", type: "link", menuItems: [] };
  ...
  const handleAddCategory = () => {
    const newCategory = { id: Date.now(), name: "New Category", icon: "🛒", slug: "new-category" };
  ...
  const getRGBFromColor = (color) => {
    ...
    if (color.startsWith("rgba")) {
      ...
      if (hex.length === 6) {
        ...
        return { r: parseInt(hex.charAt(0) + hex.charAt(0), 16), g: parseInt(hex.charAt(1) + hex.charAt(1), 16), b: parseInt(hex.charAt(2) + hex.charAt(2), 16) };
      }
  ...
  return (
    <div className="customization-panel">
      <div className="panel-header">
        <h1 className="panel-title" style={{ display: "flex", alignItems: "center", gap: "10px", fontSize: "40px" }}>
        ...
        <div className="button-group">
          ...
          {onResetToOriginal && (
            <button type="button" onClick={onResetToOriginal} className="reset-original-button">
              <DynamicIcon name="FaArrowsRotate" size={17} style={{ color: "white", marginRight: "5px" }} />
              Reset to Original Template
            </button>
          )}
          <button type="button" onClick={onExportJSON} className="export-button">
            <DynamicIcon name="IoIosSave" size={19} style={{ color: "white", marginRight: "5px" }} />
            Save
          </button>
          <button type="button" onClick={onHandlePreview} className="export-button">
            <DynamicIcon name="IoIosSave" size={19} style={{ color: "white", marginRight: "5px" }} />
            Preview Your Customization
          </button>
          {user && onExportAll && (
            <button type="button" onClick={onExportAll} className="export-all-button">
              <DynamicIcon name="IoIosSave" size={19} style={{ color: "white", marginRight: "5px" }} />
              Save All
            </button>
          )}
          {user && onLoadFromFolder && (
            <button type="button" onClick={onLoadFromFolder} className="load-folder-button">
        ...
        <div className="main-tabs">
          ...
          >
            <DynamicIcon name="IoIosColorPalette" size={20} style={{ color: "currentColor" }} />
            Colors
          </button>
          <button
            type="button"
            ...
            style={{ display: "flex", alignItems: "center", justifyContent: "center", gap: "6px" }}
          >
            <DynamicIcon name="FiEye" size={19} style={{ color: "currentColor" }} />
            Text Content
          </button>
        </div>
      </div>
      ...
      {activeMainTab === "colors" && (
        <div className="mt-20">
          ...
          {isOverlayDesign && !isShoppingCart && (
            <div className="section">
              <h3 className="section-title">
                <DynamicIcon name="IoIosColorPalette" size={25} style={{ color: "#252424" }} /> Theme Colors
              </h3>
              <ColorPickerRow
                label="Accent Color"
          ...
          {!isOverlayDesign && !isShoppingCart && (
            <div className="section">
              <h3 className="section-title">
                <DynamicIcon name="IoIosColorPalette" size={25} style={{ color: "#252424" }} /> Brand Colors
              </h3>
              <ColorPickerRow
                label="Button Background"
                color={customColors.buttonBg}
        ...
        <div className="content-container">
            ...
            <>
              ...
              <div className="section">
                <div className="flex justify-between items-center mb-4">
                  ...
                  <button type="button" onClick={handleAddCategory} className="px-3 py-1 bg-green-500 text-white rounded-lg text-sm hover:bg-green-600">
                    + Add Category
                  </button>
                </div>
                {textContent.categories.map((cat, idx) => (
                  <div key={cat.id} className="border border-gray-200 rounded-lg p-3 mb-3">
                    <div className="flex justify-between items-center mb-2">
                      ...
                      <button type="button" onClick={() => handleRemoveCategory(idx)} className="text-red-500 text-xs hover:text-red-700">
                        Remove
                      </button>
                    </div>
              ...
              <div className="section">
                <div className="flex justify-between items-center mb-4">
                  ...
                  <button type="button" onClick={handleAddProduct} className="px-3 py-1 bg-green-500 text-white rounded-lg text-sm hover:bg-green-600">
                    + Add Product
                  </button>
                </div>
                {textContent.products.map((prod, idx) => (
                  <div key={prod.id} className="border border-gray-200 rounded-lg p-3 mb-3">
                    <div className="flex justify-between items-center mb-2">
                      ...
                      <button type="button" onClick={() => handleRemoveProduct(idx)} className="text-red-500 text-xs hover:text-red-700">
                        Remove
                      </button>
                    </div>
                    <div className="grid grid-cols-2 gap-2">
          ...
          {!isShoppingCart && (
            <>
              <div className="section">
                <div className="flex justify-between items-center mb-4">
                  <h3 className="section-title mb-0">
                    <DynamicIcon name="FiMapPin" size={18} style={{ color: "#000000" }} /> Navigation Menu
                  </h3>
                  <button type="button" onClick={handleAddNavItem} className="px-3 py-1 bg-green-500 text-white rounded-lg text-sm hover:bg-green-600 transition">
                    + Add Menu Item
                  </button>
                </div>
                ...
                {textContent.navigationItems.map((item, index) => (
                  <div key={index} className="border border-gray-200 rounded-lg p-3 mb-3 bg-white">
                    <div className="flex justify-between items-center mb-3">
                      ...
                      <button type="button" onClick={() => handleRemoveNavItem(index)} className="text-red-500 text-xs hover:text-red-700 transition px-2 py-1 rounded hover:bg-red-50">
                        Remove
                      </button>
                    </div>
                      ...
                      <div className="ml-4 mt-3 p-3 bg-gray-50 rounded-lg border-l-4 border-blue-400">
                        <div className="flex justify-between items-center mb-3">
                          <label className="flex items-center gap-2 font-medium text-sm text-gray-700">
                            <DynamicIcon name="SlNotebook" size={15} style={{ color: "#000000" }} />
                            <span>Dropdown Items</span>
                          </label>
                          <button type="button" onClick={() => handleAddDropdownItem(index)} className="px-2 py-1 bg-blue-500 text-white rounded text-xs hover:bg-blue-600 transition flex items-center gap-1">
                            + Add Item
                          </button>
                        </div>
                          ...
                          {item.menuItems && item.menuItems.map((menuItem, menuIndex) => (
                            <div key={menuIndex} className="flex gap-2 items-center">
                              ...
                              <input
                                ...
                                onKeyDown={preventEnterSubmit}
                              />
                              <button type="button" onClick={() => handleRemoveDropdownItem(index, menuIndex)} className="text-red-500 text-xs hover:text-red-700 px-2 py-1 rounded hover:bg-red-50 transition" title="Remove item">
                                ✕
                              </button>
                            </div>
                          ))}
                ...
                {textContent.navigationItems.length > 0 && (
                  <div className="mt-4 p-4 bg-gray-50 rounded-xl">
                    <h4 className="text-xs font-semibold text-gray-500 mb-2 flex items-center gap-2">
                      <span><DynamicIcon name="FiEye" size={18} style={{ color: "#000000" }} /></span> Live Preview
                    </h4>
                    <div className="flex flex-wrap gap-2">
                      {textContent.navigationItems.map((item, idx) => (
                        <div key={idx} className="relative group">
                          <button type="button" className="px-3 py-1.5 bg-white border border-gray-200 rounded-lg text-sm text-gray-700 hover:border-blue-400 transition">
                            {item.label}
                          ...
                          {item.type === "dropdown" && item.menuItems && item.menuItems.length > 0 && (
                            <div className="absolute top-full left-0 mt-1 bg-white border border-gray-200 rounded-lg shadow-lg opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all z-10 min-w-[150px]">
                              {item.menuItems.map((subItem, subIdx) => (
                                <div key={subIdx} className="px-3 py-1.5 text-xs text-gray-600 hover:bg-gray-50 rounded">{subItem}</div>
                              ))}
                            </div>
                          )}
                        </div>
              ...
              <div className="section">
                <h3 className="section-title">
                  <DynamicIcon name="FiHome" size={20} style={{ color: "#000000" }} />
                  Home Section
                </h3>
                <div className="form-group">
              ...
              <div className="section">
                <h3 className="section-title">
                  <DynamicIcon name="FaBookOpen" size={20} style={{ color: "#000000" }} />
                  About Section
                </h3>
                <div className="form-group">
              ...
              <div className="section">
                <h3 className="section-title">
                  <DynamicIcon name="FaGraduationCap" size={23} style={{ color: "#000000" }} />
                  Courses Section
                </h3>
                <div className="form-group">
                  <label className="form-label">Section Title</label>
              ...
              <div className="section">
                <h3 className="section-title">
                  <DynamicIcon name="IoIosTrophy" size={23} style={{ color: "#000000" }} /> Achievements Section
                </h3>
                <div className="form-group">
                  <label className="form-label">Section Title</label>
                  <input
              ...
              <div className="section">
                <h3 className="section-title">
                  <DynamicIcon name="FiPhone" size={23} style={{ color: "#000000" }} /> Contact Section
                </h3>
                <div className="form-group">
                  <label className="form-label">Section Title</label>
                  <input
